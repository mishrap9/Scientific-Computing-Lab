...
% --- Main function ---
function next = mchoice(ng, i, j, i_comp, j_comp) % i_com, j_comp - last two computer moves
% machine next move
global samplem transm policy samplem2 transm2 % samplem2 transm2
if(ng == 1)
    % initialize matrices
    [samplem,transm]   = initmchoice;
    next = randi(3);
    [samplem2,transm2] = initmchoice; % initialize sample2 and transm2
else
    samplem  = updatesamplem(i,j,samplem);
    transm   = updatetransm(samplem);
    samplem2  = updatesamplem(i_comp,j_comp,samplem2); % update sample2
    transm2   = updatetransm(samplem2); % update transm2
    switch policy
        case 1
...
function next = predict1(j, transm)
    ...
    r = rand; % iv.1)
    prob = [param_a, param_b - param_a, 1 - param_b]; % iv.2)
    hnext_fun = [mod(j, 3) + 1, mod(j + 1, 3) + 1, j];
    hnext = hnext_fun(sum(r >= cumsum([0, prob]))); % iv.2)
