function y = adams_moulton_l1(y0, deltat, tend, eps_newton, Nmax)
    % Adams_Moulton implements the second order Adams_Moulton method for
    % the solution of the initial value problem:
    %    dy(t)/d(t) = f(y(t)), y(0)=y0,
    % using the linearisation (l1 in the WS):
    %   y_{n+1}=y_n+delta_t/2*(7*(1-y_n/10)*y_n+7*(1-y_{n+1}/10)*y_{n}).
    %
    %   Inputs:
    ...
    for i = 1 : length(t) - 1
        ...
        dG = @(x)(1 + 7 / 20 * deltat * y(i)); % the derivative is constant over the Newton iterations
        G = @(x)(x - y(i) - deltat / 2 * 7 * y(i) * (2 - (x + y(i)) / 10));
    ...
    %  Explicit formula:
    % y(i+1) = (1+7/2*deltat*(2-y(i)/10))/(1+7/20*deltat*y(i)) * y(i);
